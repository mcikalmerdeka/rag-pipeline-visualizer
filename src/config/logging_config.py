"""Centralized logging configuration for the RAG Pipeline Visualizer."""
import logging
import sys
...
def setup_logger(name: str = "rag_visualizer", log_file: str = None, level: int = logging.INFO) -> logging.Logger:
    """Setup centralized logging for the application.
    
    Args:
        name: Logger name (default: "rag_visualizer")
...
def get_logger(name: str = None) -> logging.Logger:
    ...
    Args:
        name: Logger name. If None, returns the root 'rag_visualizer' logger.
    
    Returns:
        Logger instance
    """
    if name:
        return logging.getLogger(f"rag_visualizer.{name}")
    return logging.getLogger("rag_visualizer")


# Pre-configured loggers for key components
logger_core = get_logger("core")
logger_vector = get_logger("vector_store")
logger_llm = get_logger("llm")
logger_text = get_logger("text_processing")
logger_ui = get_logger("ui")
logger_viz = get_logger("visualization")
